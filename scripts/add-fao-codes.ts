#!/usr/bin/env tsx
/**
 * Add FAO 3-alpha codes to species table
 * Runs the upsert SQL to add fao_3alpha_code column and populate it
 */
...
async function addFaoCodes() {
  console.log('🚀 Adding FAO 3-Alpha Codes to Species Table\n');
  console.log('='.repeat(60));
  
  try {
    // Read the SQL file
    const sqlPath = '/Users/damianrafferty/Downloads/upsert_fao_codes.sql';
    const sql = fs.readFileSync(sqlPath, 'utf8');
    
    console.log('\n📝 SQL Script loaded from Downloads folder');
    console.log(`   File: ${sqlPath}`);
    
    // Extract just the ALTER TABLE and UPDATE parts (skip BEGIN/COMMIT for Supabase)
    const lines = sql.split('\n');
    const alterLine = lines.find(l => l.includes('ALTER TABLE'));
    const updateStart = lines.findIndex(l => l.includes('UPDATE public.species'));
    const updateEnd = lines.findIndex(l => l.includes('COMMIT'));
    
    console.log('\n📊 Step 1: Adding fao_3alpha_code column...\n');
    
    // Note: Column must be added via Supabase SQL Editor first
    // We'll check if it exists and continue with updates
    console.log('   ℹ️  Column fao_3alpha_code should be added via Supabase SQL Editor');
    
    console.log('\n📝 Step 2: Updating FAO codes for 77 species...\n');
    
    // FAO code mappings from the SQL file
    ...
    let updateCount = 0;
    let errorCount = 0;
    const updates: Array<{ species: string; code: string }> = [];
    
    for (const [id, faoCode] of Object.entries(faoCodeMap)) {
      ...
      const { error: updateError } = await supabase
        .from('species')
        .update({ fao_3alpha_code: faoCode })
        .eq('id', id);
      
      if (updateError) {
        console.error(`   ❌ Error updating ${species.name_en}:`, updateError.message);
        errorCount++;
      } else {
        updateCount++;
        updates.push({ species: species.name_en, code: faoCode });
        if (updateCount <= 10) {
          console.log(`   ✅ ${species.name_en.padEnd(30)} → ${faoCode}`);
        }
      }
    }
    ...
    const { data: withFao, error: verifyError } = await supabase
      .from('species')
      .select('species_code, fao_3alpha_code, name_en')
      .not('fao_3alpha_code', 'is', null)
      .order('species_code');
    
    if (verifyError) throw verifyError;
    
    console.log(`   ✅ Total species with FAO codes: ${withFao?.length || 0}`);
    ...
    // Show some examples
    console.log('\n📋 Sample Results (Internal Code → FAO Code):\n');
    withFao?.slice(0, 15).forEach(s => {
      const match = s.species_code === s.fao_3alpha_code ? '✓' : ' ';
      console.log(`   ${match} ${s.species_code.padEnd(12)} → ${s.fao_3alpha_code?.padEnd(5)} | ${s.name_en}`);
    });
    
    console.log('\n');
    console.log('='.repeat(60));
    console.log('\n✅ FAO Codes Migration Complete!\n');
    console.log('Database now has:');
    console.log('  • species_code: Your internal descriptive codes');
    console.log('  • fao_3alpha_code: Official FAO 3-alpha codes');
    console.log('\nBenefits:');
    console.log('  ✅ Future-proof for data ingestion');
    console.log('  ✅ Compatible with FAO databases');
