...
interface ConfidenceResult {
  ...
  seasonality_source_confidence: number | null;
}
...
async function testLocation(rectangleCode: string, locationName: string) {
  ...
  // Call the DEFAULT function (alias to v3)
  const { data, error } = await supabase.rpc('get_fishing_confidence', {
  ...
  results.slice(0, 10).forEach((r, i) => {
    ...
    if (r.seasonality_source) {
      console.log(`      Source: ${r.seasonality_source} (confidence: ${r.seasonality_source_confidence})`);
    }
  });
...
async function runTests() {
  console.log('╔═══════════════════════════════════════════════════════════════════════════════╗');
  console.log('║  DEFAULT get_fishing_confidence() - Multi-Location Test                      ║');
  console.log('║  Verifying V3 alias is active with regional seasonality                      ║');
  console.log('╚═══════════════════════════════════════════════════════════════════════════════╝');
  console.log(`\n🗓️  Testing for: November 2025`);
  console.log(`⚙️  Function: get_fishing_confidence (should use V3)`);
  ...
  console.log('='.repeat(80));
  console.log('\n✨ If you see seasonality data above, V3 is now the default! ✨\n');
}
