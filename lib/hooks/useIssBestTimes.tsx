...
export function IssSightingNote({ data }: { data: IssResult[] | null }) {
  ...
  return (
    <div className="iss-sighting-note" style={{ display: 'flex', alignItems: 'center', margin: '16px 0', gap: 12 }}>
      <img src="/satellite_iss.png" alt="ISS" style={{ width: 40, height: 40, flexShrink: 0 }} />
      <div>
        <strong>ISS visible tonight:</strong><br />
        Look up at <b>{range}</b> for about <b>{minutes} minute{minutes !== 1 ? 's' : ''}</b>.<br />
        The International Space Station will be bright and moving quickly across the sky.
      </div>
    </div>
  );
}
