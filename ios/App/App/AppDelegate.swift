...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    private func registerBackgroundTasks() {
        ...
        BGTaskScheduler.shared.register(
            forTaskWithIdentifier: "eu.fishfindr.app.refresh",
            using: nil
        ) { task in
    ...
    func scheduleAppRefresh() {
        let request = BGAppRefreshTaskRequest(identifier: "eu.fishfindr.app.refresh")
...
extension AppDelegate: MessagingDelegate {
    ...
    func messaging(_ messaging: Messaging, didReceiveRegistrationToken fcmToken: String?) {
        ...
        Messaging.messaging().subscribe(toTopic: "findr-all") { error in
            if let error = error {
                print("❌ Failed to subscribe to findr-all topic: \(error.localizedDescription)")
            } else {
                print("✅ Subscribed to findr-all topic")
            }
        }
