...
const RecentCatchesWidget = dynamic(() => import('../../components/findr/RecentCatchesWidget').then(mod => ({ default: mod.RecentCatchesWidget })), { ssr: false, loading: () => null });
    ...
    {catches.map((catchEntry) => {
      // Try to find a species image by common name (case-insensitive match)
      const speciesInfo = Object.values(SPECIES_IMAGE_MAP).find(
        s => s.name.toLowerCase() === catchEntry.species_common_name.toLowerCase()
      );
      return (
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <h3 className="text-xl font-bold text-primary flex items-center gap-2">
                  {speciesInfo && speciesInfo.thumb ? (
                    <img
                      src={speciesInfo.thumb}
                      alt={catchEntry.species_common_name}
                      className="w-7 h-7 rounded object-cover border border-base-300 bg-base-100"
                    />
                  ) : (
                ...
                {catchEntry.bait_used && (
                  <div className="mt-2 flex items-center gap-2">
                    ...
                    <span className="badge badge-info badge-outline text-sm font-medium px-3 py-1">
                      {catchEntry.bait_used}
                    </span>
                  </div>
                )}
                ...
                {catchEntry.habitat && (
                  <div className="mt-1 flex items-center gap-2">
                    ...
                    <span className="badge badge-success badge-outline text-sm font-medium px-3 py-1">
                      {catchEntry.habitat}
                    </span>
                  </div>
                )}
