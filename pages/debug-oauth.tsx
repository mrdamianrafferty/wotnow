...
export default function DebugOAuth() {
  ...
  useEffect(() => {
    const checkStorage = () => {
      const allLocalStorage: StorageMap = {};
      ...
      const allSessionStorage: StorageMap = {};
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      <div className="card bg-base-200">
        <div className="card-body">
          ...
          <pre className="text-xs overflow-auto bg-base-300 p-4 rounded max-h-60">{JSON.stringify(info?.supabaseKeys ?? [], null, 2)}</pre>
          {info && info.supabaseKeys.length === 0 && <p className="text-warning">⚠️ No Supabase keys found in localStorage!</p>}
        </div>
      </div>
      ...
      <div className="card bg-base-200">
        <div className="card-body">
          ...
          <pre className="text-xs overflow-auto bg-base-300 p-4 rounded max-h-60">{JSON.stringify(info?.localStorage ?? {}, null, 2)}</pre>
        </div>
      </div>
      ...
      <div className="card bg-base-200">
        <div className="card-body">
          ...
          <pre className="text-xs overflow-auto bg-base-300 p-4 rounded max-h-60">{JSON.stringify(info?.sessionStorage ?? {}, null, 2)}</pre>
        </div>
      </div>
      ...
      <div className="card bg-warning text-warning-content">
        <div className="card-body">
          ...
          <ul className="list-disc list-inside space-y-2 text-sm">
            <li>
              <strong>No Supabase keys in localStorage:</strong> PKCE verifier not being stored
            </li>
            <li>
              <strong>Wrong domain in localStorage keys:</strong> OAuth initiated from different domain
            </li>
            <li>
              <strong>Supabase Site URL mismatch:</strong> Check Supabase Dashboard → Authentication → URL Configuration
            </li>
            <li>
              <strong>Required Site URL:</strong> Must be <code>https://www.godaisy.io</code> (exact match including www)
            </li>
          </ul>
        </div>
      </div>
