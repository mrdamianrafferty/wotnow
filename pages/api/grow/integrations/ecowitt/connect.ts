...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'POST') {
    ...
    if (!apiKey) {
      return res.status(400).json({ error: 'Please paste your Ecowitt connection key.' });
    }
    ...
    try {
      ...
      if (!validation.valid || !validation.devices) {
        return res.status(400).json({
          error: validation.error || 'We couldn\'t verify this key. Please check you copied it correctly from ecowitt.net.',
        });
      }
      ...
      if (mac) {
        ...
        if (!found) {
          return res.status(400).json({ error: 'We connected to your account but couldn\'t find any devices. Make sure your Ecowitt station is set up and online.' });
        }
