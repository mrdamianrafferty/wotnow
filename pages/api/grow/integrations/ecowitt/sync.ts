/**
 * /api/grow/integrations/ecowitt/sync
 *
 * Sync data from a connected Ecowitt weather station.
 *
 * POST: Fetch latest observations and store in database
 * GET: Get latest synced data for a station
 *
 * @module pages/api/grow/integrations/ecowitt/sync
 */

import type { NextApiRequest, NextApiResponse } from 'next';
import { createClient } from '@supabase/supabase-js';
import { fetchEcowittData } from '@/lib/grow/ecowitt';
import {
  getIntegrationToken,
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (integration.integration_type !== 'ecowitt') {
    return res.status(400).json({ error: 'This endpoint is for Ecowitt integrations only' });
  }
  ...
  if (req.method === 'POST') {
    try {
      ...
      const result = await fetchEcowittData(integration.station_id, apiKey);

      if (!result.success || !result.data) {
        return res.status(502).json({
          error: result.error || 'Failed to fetch Ecowitt data',
        });
      }
      ...
      console.error('[Ecowitt Sync] Error:', error);
    ...
    try {
      ...
      if (dataError && dataError.code !== 'PGRST116') {
        console.error('[Ecowitt Sync] Fetch error:', dataError);
