...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'POST') {
    ...
    if (!apiKey || !apiSecret) {
      return res.status(400).json({ error: 'Please paste both your key and secret from WeatherLink.' });
    }
    ...
    try {
      ...
      if (!validation.valid || !validation.stations) {
        return res.status(400).json({
          error: validation.error || 'We couldn\'t verify these credentials. Please check you copied them correctly from weatherlink.com.',
        });
      }
      ...
      if (stationId) {
        ...
        if (!found) {
          return res.status(400).json({ error: 'We connected to your account but couldn\'t find any stations. Make sure your Davis station is set up and online.' });
        }
