/**
 * /api/grow/integrations/weatherlink/sync
 *
 * Sync data from a connected Davis WeatherLink weather station.
 *
 * POST: Fetch latest observations and store in database
 * GET: Get latest synced data for a station
 *
 * @module pages/api/grow/integrations/weatherlink/sync
 */

import type { NextApiRequest, NextApiResponse } from 'next';
import { createClient } from '@supabase/supabase-js';
import { fetchWeatherLinkCurrent } from '@/lib/grow/weatherlink';
import {
  getIntegrationToken,
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'POST') {
    try {
      // Get the stored credentials (format: apiKey|apiSecret)
      const credentials = await getIntegrationToken(supabase, integrationId);
      ...
      if (!apiKey || !apiSecret) {
        return res.status(401).json({
          error: 'Invalid credentials format. Please reconnect.',
        });
      }
      ...
      if (!result.success || !result.data) {
        return res.status(502).json({
          error: result.error || 'Failed to fetch WeatherLink data',
        });
      }

      // Store the data with soil sensors
      ...
      console.error('[WeatherLink Sync] Error:', error);
    ...
    try {
      ...
      if (dataError && dataError.code !== 'PGRST116') {
        console.error('[WeatherLink Sync] Fetch error:', dataError);
