...
/**
 * Enhanced rainfall description that considers both daily aggregate and hourly forecasts
 * 
 * @param mm Daily aggregate rainfall in mm
 * @param hourlyData Optional array of hourly rainfall data 
 * @returns String description with time context if available
 */
export function getRainfallDescriptionWithHourly(
  mm: number, 
  hourlyData?: { time?: string | number; rain?: number; dt?: number; precipitation?: number }[]
): string {
  // If no hourly data available, fall back to simple description
  if (!hourlyData || !Array.isArray(hourlyData) || hourlyData.length === 0) {
    return getRainfallDescription(mm);
  }

  // Check if daily is 0 but hourly has rain
  if (mm === 0 || mm < 0.1) {
    // Extract rain values from hourly data, handling different property names
    const hourlyRainValues = hourlyData.map(hour => {
      const rainVal = hour.rain ?? hour.precipitation ?? 0;
      return {
        rain: typeof rainVal === 'number' ? rainVal : 0,
        time: hour.time ?? hour.dt,
      };
    });

    // Find hours with rain
    const rainyHours = hourlyRainValues.filter(h => h.rain > 0);
    
    if (rainyHours.length === 0) {
      return 'No rain';
    }

    // Find the first hour with rain and format the time
    const firstRainHour = rainyHours[0];
    let timeStr = '';
    
    if (firstRainHour.time) {
      try {
        const timestamp = typeof firstRainHour.time === 'string' 
          ? new Date(firstRainHour.time).getTime() 
          : typeof firstRainHour.time === 'number' 
            ? firstRainHour.time * 1000  // Convert from Unix seconds to milliseconds
            : null;
        
        if (timestamp) {
          const rainTime = new Date(timestamp);
          const hours = rainTime.getHours();
          // Format time in 12-hour format with am/pm
          timeStr = `${hours % 12 || 12}${hours < 12 ? 'am' : 'pm'}`;
        }
      } catch (e) {
        console.warn('Failed to parse rain time:', e);
      }
    }

    // Get total rain amount for rain intensity description
    const totalRain = rainyHours.reduce((sum, h) => sum + h.rain, 0);
    const rainIntensity = 
      totalRain < 1 ? 'light showers' : 
      totalRain < 5 ? 'showers' : 
      totalRain < 15 ? 'rain' : 'heavy rain';

    return timeStr 
      ? `Mostly dry, chance of ${rainIntensity} around ${timeStr}` 
      : `Mostly dry, chance of ${rainIntensity} later`;
  }

  // If daily rain is significant, use the regular description
  return getRainfallDescription(mm);
}
