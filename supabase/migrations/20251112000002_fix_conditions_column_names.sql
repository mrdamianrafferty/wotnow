-- Fix RPC function to use correct column names from findr_conditions_latest table
-- The table has: sea_temp_c, water_temp_c, salinity_psu (NOT temperature_c, salinity_ppt, substrate)
...
COMMENT ON FUNCTION get_environmental_predictions_enhanced IS 'Enhanced confidence scoring with time-of-day awareness. FIXED: Uses correct column names from findr_conditions_latest table (sea_temp_c/water_temp_c, salinity_psu). Substrate comes from parameter only as it does not exist in conditions table.';
