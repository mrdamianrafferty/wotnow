...
WHERE scientific_name = 'Pollachius pollachius'
   ...
   OR name_en ILIKE '%pollack%' OR name_en ILIKE '%pollock%';
...
WHERE scientific_name = 'Labrus bergylta'
   ...
   OR name_en ILIKE '%ballan%wrasse%';
...
WHERE scientific_name = 'Pollachius virens'
   OR species_code = 'POK'
   OR name_en ILIKE '%coalfish%' OR name_en ILIKE '%saithe%';
...
WHERE scientific_name = 'Trachurus trachurus'
   OR species_code = 'HOM'
   OR name_en ILIKE '%horse%mackerel%' OR name_en ILIKE '%scad%';
...
WHERE scientific_name = 'Sardina pilchardus'
   OR species_code = 'PIL'
   OR name_en ILIKE '%pilchard%' OR name_en ILIKE '%sardine%';
...
INSERT INTO species (
    ...
    -- Seabreams
    ('wht-bream', 'Diplodus sargus', 'White Seabream', 'Sargo', 'Sar Commun', 'Sarago Maggiore', 4, 
     ARRAY['float fishing', 'bottom fishing', 'light tackle'], 0, 20, ARRAY['rocky reefs', 'seagrass beds'], false, false),
    
    ('2bd-bream', 'Diplodus vulgaris', 'Two-banded Seabream', 'Mojarra', 'Sar à Tête Noire', 'Sarago Fasciato', 4,
     ARRAY['float fishing', 'bottom fishing', 'light tackle'], 0, 20, ARRAY['rocky reefs', 'seagrass beds'], false, false),
    ...
    ('sadd-bream', 'Oblada melanura', 'Saddled Seabream', 'Oblada', 'Oblade', 'Occhiata', 3,
    ...
    -- Mackerels & Tunas
    ('chub-mack', 'Scomber colias', 'Atlantic Chub Mackerel', 'Estornino', 'Maquereau Espagnol', 'Lanzardo', 4,
    ...
    -- Barracudas
    ('yel-cuda', 'Sphyraena viridensis', 'Yellowmouth Barracuda', 'Barracuda Amarilla', 'Bécune Yellowmouth', 'Barracuda Bocca Gialla', 3,
    ...
    -- Groupers
    ('dusk-group', 'Epinephelus marginatus', 'Dusky Grouper', 'Mero', 'Mérou Brun', 'Cernia Bruna', 5,
     ARRAY['bottom fishing', 'jigging', 'live bait'], 15, 100, ARRAY['caves', 'rocky reefs', 'wrecks'], false, false),
    
    ('whit-group', 'Epinephelus aeneus', 'White Grouper', 'Cherne Blanco', 'Mérou Blanc', 'Cernia Bianca', 5,
     ARRAY['bottom fishing', 'jigging', 'live bait'], 15, 80, ARRAY['wrecks', 'reef edges'], false, false),
    
    -- Scorpionfish
    ('red-scorp', 'Scorpaena scrofa', 'Red Scorpionfish', 'Cabracho', 'Chapon', 'Scorfano Rosso', 3,
...
WHERE scientific_name = 'Diplodus sargus'
   OR name_en ILIKE '%white%seabream%';
...
WHERE scientific_name = 'Diplodus vulgaris'
   OR name_en ILIKE '%two%banded%seabream%';
...
WHERE scientific_name = 'Pagellus erythrinus'
   OR name_en ILIKE '%common%pandora%';
...
WHERE scientific_name = 'Oblada melanura'
   OR name_en ILIKE '%saddled%seabream%' OR name_en ILIKE '%saddled%bream%';
...
WHERE scientific_name = 'Trachurus mediterraneus'
   OR name_en ILIKE '%mediterranean%scad%' OR name_en ILIKE '%mediterranean%horse%mackerel%';
...
WHERE scientific_name = 'Scomber colias'
   OR name_en ILIKE '%chub%mackerel%';
...
WHERE scientific_name = 'Sarda sarda'
   OR name_en ILIKE '%atlantic%bonito%' OR name_en ILIKE '%bonito%';
...
WHERE scientific_name = 'Lichia amia'
   OR name_en ILIKE '%leerfish%' OR name_en ILIKE '%garrick%';
...
WHERE scientific_name = 'Sphyraena viridensis'
   OR name_en ILIKE '%yellowmouth%barracuda%';
...
WHERE scientific_name = 'Sphyraena sphyraena'
   OR name_en ILIKE '%european%barracuda%' OR name_en ILIKE '%barracuda%';
...
WHERE scientific_name = 'Epinephelus marginatus'
   OR name_en ILIKE '%dusky%grouper%';
...
WHERE scientific_name = 'Epinephelus aeneus'
   OR name_en ILIKE '%white%grouper%';
...
WHERE scientific_name = 'Scorpaena scrofa'
   OR name_en ILIKE '%red%scorpionfish%' OR name_en ILIKE '%large%scaled%scorpionfish%';
...
WHERE scientific_name = 'Argyrosomus regius'
   OR name_en ILIKE '%meagre%' OR name_en ILIKE '%shi%drum%';
