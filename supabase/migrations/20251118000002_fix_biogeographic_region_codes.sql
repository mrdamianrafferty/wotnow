-- Fix Biogeographic Region Codes
-- November 18, 2025
-- Bug Fix: Region code mismatch between migration and database
-- Issue: Migration used codes like 'BIS', 'IBR', 'NEA' but database has 'NE_Atlantic', 'NW_Atlantic', etc.
-- Solution: Update region mapping to use actual database codes
...
BEGIN
  ...
  -- FIXED: Now maps to actual database region codes
  ...
  -- **CRITICAL FIX: Added biogeographic region filtering with correct codes**
  species_scores AS (
    SELECT
      s.id AS species_id,
    ...
    WHERE s.species_code IS NOT NULL
      -- **BIOGEOGRAPHIC FILTER with FIXED region codes**
      AND (
...
COMMENT ON FUNCTION get_fishing_confidence_v3 IS 'Confidence V3 with biogeographic filtering using correct database region codes (FIXED 2025-11-18). Now maps IBI→NE_Atlantic, NWS→NE_Atlantic, MED→Mediterranean. Prevents Pacific species from appearing in Atlantic waters and vice versa.';
