-- Fix substrate matching in bite score calculation
-- The substrate_matches CTE was incorrectly trying to SELECT ss.substrate_type
-- The species_substrates table actually has boolean columns: has_sand, has_rock, has_mud, has_gravel

DROP FUNCTION IF EXISTS get_environmental_predictions_enhanced(text, date, numeric, numeric, text, integer, numeric, numeric);
...
DO $$
BEGIN
  RAISE NOTICE '✅ Substrate matching fixed in bite score calculation!';
END $$;
