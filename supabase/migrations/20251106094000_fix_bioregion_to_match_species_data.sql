-- Fix biogeographic region mapping to match ACTUAL species data
-- Date: 2025-11-06 09:40
--
-- ROOT CAUSE: Species table contains 'NE_Atlantic', 'Mediterranean', 'NW_Atlantic', etc.
-- Our migration was mapping to 'Celtic Sea', 'English Channel', 'Bay of Biscay'
-- These don't match! Need to map ALL European rectangles to 'NE_Atlantic' or 'Mediterranean'
...
BEGIN
  ...
  -- Map to regions that ACTUALLY EXIST in species.biogeographic_regions
  -- Based on actual database query: species have 'NE_Atlantic', 'Mediterranean', 'NW_Atlantic', etc.
  rectangle_region := CASE
    -- Mediterranean (07xx, 08xx)
    WHEN target_rectangle LIKE '07%' OR target_rectangle LIKE '08%' THEN 'Mediterranean'
    -- All other European waters → NE_Atlantic
    ELSE 'NE_Atlantic'
  END;
