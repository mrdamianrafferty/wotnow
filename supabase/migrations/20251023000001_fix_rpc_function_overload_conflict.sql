-- Fix RPC function overload conflict
-- Issue: Multiple versions of get_environmental_predictions_enhanced exist with different signatures
-- Solution: Drop ALL versions and recreate with correct signature

-- Drop all versions of the function (CASCADE removes dependencies)
DROP FUNCTION IF EXISTS get_environmental_predictions_enhanced CASCADE;

-- Recreate the function with correct signature and return type
...
DO $$
BEGIN
  RAISE NOTICE '✅ RPC function conflict resolved - all overloaded versions dropped and recreated cleanly';
END $$;
