-- Restore enhanced RPC fallback when latest conditions are missing
-- Builds on emergency fix while keeping new scoring fields intact

-- EMERGENCY FIX: Query findr_conditions_latest instead of findr_conditions_snapshots
-- The snapshots table is empty but the view has the data (283 records)
...
BEGIN
  ...
  WITH recent_conditions AS (
    -- Ensure we always have at least one row even if no latest conditions exist
    SELECT
...
DO $$
BEGIN
  RAISE NOTICE '🚨 EMERGENCY FIX APPLIED: Now querying findr_conditions_latest instead of findr_conditions_snapshots';
END $$;
