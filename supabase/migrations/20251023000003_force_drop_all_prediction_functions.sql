-- Aggressively drop all versions of prediction functions
...
DECLARE
  r RECORD;
BEGIN
  -- Drop all functions matching the pattern
  FOR r IN
    SELECT
      proname,
      oid::regprocedure as function_sig
    FROM pg_proc
    WHERE proname = 'get_environmental_predictions_enhanced'
  LOOP
    RAISE NOTICE 'Dropping function: %', r.function_sig;
    EXECUTE format('DROP FUNCTION IF EXISTS %s CASCADE', r.function_sig);
  END LOOP;
END$$;

-- Now create the function with the correct signature
-- Using explicit type casting to ensure all columns match exactly
...
RETURNS TABLE (
  species_id uuid,
  species_code text,  -- Changed to text to avoid varchar length mismatch
...
BEGIN
  ...
  biogeochemical_enhancements AS (
    SELECT
      s.id as species_id,
      s.species_code::text,  -- Cast to text
      s.name_en::text,       -- Cast to text
      s.scientific_name::text, -- Cast to text
      s.playful_bio_en,
...
DO $$
BEGIN
  RAISE NOTICE '✅ All prediction function versions dropped and recreated with text types to avoid varchar length mismatches';
END $$;
