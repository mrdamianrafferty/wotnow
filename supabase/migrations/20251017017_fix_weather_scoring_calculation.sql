-- Fix weather scoring calculation
-- The wind_weight and pressure_weight were being used as multipliers (0-1 scale)
-- but should be used to weight the final score (0-10 scale)
  ...
  weather_matches AS (
    SELECT
      ...
      CASE
        ...
        -- Calculate weighted average of wind and pressure
        ELSE
          ROUND(
            (
...
DO $$
BEGIN
  RAISE NOTICE '✅ Fixed weather scoring calculation!';
  RAISE NOTICE 'Now using weighted average instead of simple multiplication';
  RAISE NOTICE 'Formula: (wind_score * wind_weight + pressure_score * pressure_weight) / (wind_weight + pressure_weight)';
END $$;
