-- Extend environmental data fallback period from 7 days to 30 days
-- This ensures predictions always show using the last known good data
-- instead of returning empty results when fresh data is unavailable
...
BEGIN
  ...
  WITH recent_conditions AS (
    -- Get most recent environmental data within 30 days (extended from 7)
    SELECT 
      rectangle_code,
...
COMMENT ON FUNCTION get_environmental_predictions_basic IS 'Biogeographically-aware confidence scoring with 30-day data fallback. Uses last known good data within 30 days to ensure predictions always available.';
