-- Lower default temp_score when temperature data is unavailable
--
-- BEFORE: temp_score defaults to 10 when env_temperature IS NULL
-- AFTER:  temp_score defaults to 5 when env_temperature IS NULL
--
-- This makes it more obvious when predictions are based on limited data
-- Rectangles without temperature will show ~75% confidence instead of 80%
    ...
    final_scores AS (
      SELECT
        ...
        CASE
          ...
          ELSE 5  -- CHANGED FROM 10 TO 5 to indicate limited data quality
        END as temp_score,
