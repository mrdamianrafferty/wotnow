-- Species-Specific Bite Score Calculation
--
-- PROBLEM: Current bite_score is mostly location-based, not species-specific
-- All species at the same location get nearly identical bite scores
--
-- SOLUTION: Use species sensitivity columns to make scores vary by species:
-- - is_night_species: Dawn/dusk/night feeders get higher scores at those times
-- - tide_sensitivity: Species with high sensitivity get boost near tide changes
-- - wind_sensitivity: Calm-water species penalized in high wind
-- - pressure_sensitivity: Affects how weather score is weighted
-- - temperature_sensitivity: Affects how temp matching is weighted
  ...
  IF has_grid_data THEN
    ...
    final_scores AS (
      SELECT
        ...
        -- TODO: Enhance with actual wind/pressure data from grid_conditions
