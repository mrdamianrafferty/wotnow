-- Fix biogeographic region mapping to use GRANULAR regions that match species data
-- Date: 2025-11-06 09:00
--
-- ROOT CAUSE: Previous migrations mapped ICES rectangles to 'NE_Atlantic' and 'Mediterranean',
-- but species.biogeographic_regions contains GRANULAR values like 'Atlantic', 'North Sea',
-- 'Celtic Sea', 'English Channel', 'Irish Sea', 'Bay of Biscay', 'IBI', etc.
--
-- The filter "rectangle_region = ANY(s.biogeographic_regions)" was failing because:
--   'NE_Atlantic' ≠ 'Atlantic', 'North Sea', 'Celtic Sea', etc.
--
-- Only species with NULL biogeographic_regions passed the filter (e.g., mullet).
--
-- SOLUTION: Map ICES rectangles to the GRANULAR region names that exist in species data.
...
DROP FUNCTION IF EXISTS get_environmental_predictions_enhanced(text, date, numeric, numeric, text, numeric, numeric, numeric, text, numeric) CASCADE;
...
BEGIN
  ...
  -- Map ICES rectangle codes to GRANULAR biogeographic regions that match species data
  -- These regions match what's actually in the species.biogeographic_regions column:
  -- 'Atlantic', 'Mediterranean', 'North Sea', 'Celtic Sea', 'English Channel',
  -- 'Irish Sea', 'Bay of Biscay', 'IBI', 'Baltic', 'Norwegian waters'

  rectangle_region := CASE
    -- Mediterranean (07xx, 08xx)
    WHEN target_rectangle LIKE '07%' OR target_rectangle LIKE '08%' THEN 'Mediterranean'

    -- Bay of Biscay and Iberian waters (20xx-29xx)
    WHEN target_rectangle LIKE '20%' OR target_rectangle LIKE '21%' THEN 'Bay of Biscay'
    WHEN target_rectangle LIKE '22%' OR target_rectangle LIKE '23%' THEN 'Bay of Biscay'
    WHEN target_rectangle LIKE '24%' OR target_rectangle LIKE '25%' THEN 'IBI'  -- Iberian coast
    WHEN target_rectangle LIKE '26%' OR target_rectangle LIKE '27%' THEN 'IBI'
    WHEN target_rectangle LIKE '28%' OR target_rectangle LIKE '29%' THEN 'IBI'

    -- English Channel and Celtic Sea (30xx-34xx)
    WHEN target_rectangle LIKE '30%' OR target_rectangle LIKE '31%' THEN 'English Channel'
    WHEN target_rectangle LIKE '32%' THEN 'Celtic Sea'
    WHEN target_rectangle LIKE '33%' THEN 'Celtic Sea'
    WHEN target_rectangle LIKE '34%' THEN 'English Channel'

    -- Irish Sea and Celtic Sea (35xx-39xx)
    WHEN target_rectangle LIKE '35%' OR target_rectangle LIKE '36%' THEN 'Irish Sea'
    WHEN target_rectangle LIKE '37%' THEN 'Celtic Sea'
    WHEN target_rectangle LIKE '38%' THEN 'Irish Sea'
    ...
    -- Norwegian waters and Barents Sea (50xx-65xx)
    ...
    -- Default to Atlantic for unspecified European waters
    ELSE 'Atlantic'
  END;
  ...
  bio_band_matches AS (
    SELECT
      be_outer.species_id,
      CASE
        WHEN be_outer.guild = 'pelagic' THEN (
          CASE WHEN be_outer.chlorophyll_mg_m3 BETWEEN 0.5 AND 2.0 THEN 35
               WHEN be_outer.chlorophyll_mg_m3 BETWEEN 0.3 AND 3.0 THEN 28
               WHEN be_outer.chlorophyll_mg_m3 IS NOT NULL THEN 20
               ELSE 15
          END
        )
        WHEN be_outer.guild = 'reef_kelp' THEN (
          CASE WHEN be_outer.dissolved_oxygen_mg_l > 7.0 THEN 35
               WHEN be_outer.dissolved_oxygen_mg_l > 6.0 THEN 28
               WHEN be_outer.dissolved_oxygen_mg_l IS NOT NULL THEN 20
               ELSE 15
          END
        )
        WHEN be_outer.guild = 'benthic' THEN (
          CASE WHEN be_outer.dissolved_oxygen_mg_l > 6.5 THEN 35
               WHEN be_outer.dissolved_oxygen_mg_l > 5.5 THEN 28
               WHEN be_outer.dissolved_oxygen_mg_l IS NOT NULL THEN 20
               ELSE 15
          END
        )
        WHEN be_outer.guild = 'surf_estuary' THEN (
          CASE WHEN be_outer.salinity_psu BETWEEN 25 AND 35 THEN 35
               WHEN be_outer.salinity_psu IS NOT NULL THEN 25
               ELSE 15
          END
        )
        WHEN be_outer.guild = 'cephalopod' THEN (
          CASE WHEN be_outer.chlorophyll_mg_m3 BETWEEN 0.4 AND 1.5 THEN 35
               WHEN be_outer.chlorophyll_mg_m3 IS NOT NULL THEN 25
               ELSE 15
          END
        )
        ELSE 20
  ...
  temp_matches AS (
    SELECT
      ...
      ROUND(
        CASE
          WHEN be.sea_temp_c IS NULL THEN 18
          WHEN be.temp_opt_c IS NULL THEN 18
          WHEN ABS(be.sea_temp_c - be.temp_opt_c) <= 2 THEN 30
          WHEN ABS(be.sea_temp_c - be.temp_opt_c) <= 4 THEN 25
          WHEN ABS(be.sea_temp_c - be.temp_opt_c) <= 6 THEN 20
          WHEN ABS(be.sea_temp_c - be.temp_opt_c) <= 8 THEN 15
          ELSE 10
        END * COALESCE(be.temp_weight, 0.25)
