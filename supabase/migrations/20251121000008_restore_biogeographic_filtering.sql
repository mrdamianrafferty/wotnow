-- Restore Biogeographic Filtering to V3
-- November 21, 2025
-- Restores the November 18th biogeographic filtering that was lost in the revert
-- Prevents Pacific species from appearing in Atlantic waters and vice versa
...
BEGIN
  ...
  -- Maps to actual database region codes (NE_Atlantic, Mediterranean, etc.)
  rectangle_regions AS (
    SELECT UNNEST(
      CASE cmems_region
        ...
        -- Fallback: Try to use region field directly
        ELSE ARRAY[]::TEXT[]
      END
  ...
  -- **BIOGEOGRAPHIC FILTER: Only include species that match the rectangle's region**
  species_scores AS (
    SELECT
      s.id AS species_id,
    ...
    WHERE s.species_code IS NOT NULL
      -- **BIOGEOGRAPHIC FILTER: Only include species that match the rectangle's biogeographic region(s)**
      AND (
        -- Species with NULL or empty biogeographic_regions are included (no regional restriction)
        s.biogeographic_regions IS NULL
        OR array_length(s.biogeographic_regions, 1) IS NULL
        -- OR species biogeographic_regions overlaps with rectangle's biogeographic regions
  ...
  with_seasonality AS (
    ...
    LEFT JOIN rectangle_regions rr ON TRUE
...
COMMENT ON FUNCTION get_fishing_confidence_v3 IS 'Confidence V3 with biogeographic filtering AND regional seasonality (RESTORED: Nov 21, 2025). Maps CMEMS regions to database biogeographic regions (IBI→NE_Atlantic, NWS→NE_Atlantic, MED→Mediterranean, BAL→all species). Filters species by biogeographic_regions array overlap. Prevents Pacific species from appearing in Atlantic waters and vice versa.';
