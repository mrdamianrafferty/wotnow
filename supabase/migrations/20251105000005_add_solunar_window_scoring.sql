-- Phase 2: Add Solunar Window Scoring to Bite Score
-- Migration: 20251105000005_add_solunar_window_scoring.sql
-- Description: Enhance bite score with Solunar Theory major/minor period bonuses
-- Related: docs/PHASE_2_PRESSURE_CLOUD_BITE_SCORE.md section 2.7

-- Drop existing function to recreate with Solunar scoring
  ...
  IF has_grid_data THEN
    ...
    final_scores AS (
      SELECT
        ...
        -- NEW: SOLUNAR SCORE - Bonus for major/minor feeding periods
        -- Major period (moon overhead/underfoot): ±1hr from transit = 5 pts
        -- Minor period (moonrise/moonset): ±1hr = 4 pts
        -- High illumination (full moon) amplifies the bonus
        (
    ...
    SELECT
      ...
      -- Calculate bite_score (sum of all components including Solunar bonus)
      (
        fs.temp_score + fs.light_score + fs.lunar_score + fs.solunar_score + fs.weather_score +
        fs.cloud_score + fs.light_cloud_bonus + fs.tidal_score +
        fs.substrate_score + fs.depth_score + fs.habitat_bonus
      ...
      'CMEMS+Findr+Solunar' as data_source
...
'Global fishing predictions with Solunar Theory major/minor period bonuses. Uses moon transit times to identify peak feeding windows (major: ±1hr from transit, minor: ±1hr from moonrise/moonset). Bonus amplified by moon illumination (full moon = stronger effect).';
