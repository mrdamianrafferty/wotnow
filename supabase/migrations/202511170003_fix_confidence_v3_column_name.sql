-- Fix column name in Confidence V3 function
-- November 17, 2025
-- Fixes: 'code' → 'rectangle_code' in ices_rectangles WHERE clause
...
COMMENT ON FUNCTION get_fishing_confidence_v3 IS 'Confidence V3 with regional seasonality integration (FIXED: rectangle_code column name). Uses species_region_seasonality table for per-region seasonal weights. Joins on (species.id, ices_rectangles.region). Returns seasonally-adjusted confidence_percent multiplied by seasonal_weight * availability_multiplier.';
