-- Fix RPC function to use correct column name: "region" instead of "biogeographic_region"
-- The ices_rectangles table has a "region" column, not "biogeographic_region"
...
COMMENT ON FUNCTION get_environmental_predictions_enhanced IS 'Enhanced confidence scoring with time-of-day awareness. Uses diurnal_sensitivity to boost dawn/dusk feeders during crepuscular hours and nocturnal species at night. Includes lat/lon-based substrate and depth scoring. FIXED: Uses correct "region" column instead of non-existent "biogeographic_region" column.';
