-- Add tide scoring to bite score calculation
-- This completes the bite score algorithm by enabling the tide factor
...
BEGIN
  ...
  tide_matches AS (
    SELECT
      ...
      ROUND(
        CASE
          -- No tide data provided - use neutral score
          WHEN current_tide_stage IS NULL THEN 15
          -- Tide data available - calculate based on species preferences
          WHEN current_tide_stage = ANY(be.preferred_tide_stage) THEN
            -- Perfect tide stage
            CASE
              -- Apply slack penalty if species avoids slack water
              WHEN be.flow_preference = 'slack_avoid'
                AND current_flow_speed_ms IS NOT NULL
                AND current_flow_speed_ms < COALESCE(be.slack_threshold_ms, 0.25) THEN 25
              -- Normal preferred stage score
              ELSE 40
            END
          -- Not preferred stage but check flow preferences
          WHEN current_flow_speed_ms IS NOT NULL THEN
            CASE
              -- Species prefers gentle flow and we have it
              WHEN be.flow_preference = 'gentle' AND current_flow_speed_ms < 0.3 THEN 25
              -- Species prefers strong flow and we have it
              WHEN be.flow_preference = 'strong' AND current_flow_speed_ms > 0.8 THEN 25
              -- Moderate flow preference
              WHEN be.flow_preference = 'moderate' AND current_flow_speed_ms BETWEEN 0.3 AND 0.8 THEN 20
              -- Wrong flow for species
              ELSE 10
            END
          -- Wrong tide stage, no flow data
          ELSE 10
        END * COALESCE(be.tide_weight, 0.30)
      )::integer as tide_score
    FROM biogeochemical_enhancements be
  ),
  habitat_bonuses AS (
    SELECT
  ...
  SELECT
    ...
    -- Bite Score Calculation with REAL Tide Data!
    LEAST(
      (
        COALESCE(tm.temp_score, 0) +
        COALESCE(tdm.tide_score, 0) +  -- NOW ACTIVE!
...
DO $$
BEGIN
  RAISE NOTICE '✅ Tide Scoring ACTIVATED in get_environmental_predictions_enhanced!';
  RAISE NOTICE '';
  RAISE NOTICE 'NEW Parameters:';
  RAISE NOTICE '  • current_tide_stage: early_flood | mid_flood | high | early_ebb | mid_ebb | low_slack';
  RAISE NOTICE '  • current_flow_speed_ms: Flow speed in meters/second';
  RAISE NOTICE '';
  RAISE NOTICE 'Tide Scoring Logic:';
  RAISE NOTICE '  • Perfect stage match (in preferred_tide_stage): 40 points × tide_weight';
  RAISE NOTICE '  • Slack penalty (if flow_preference = slack_avoid): -15 points';
  RAISE NOTICE '  • Flow match bonus (gentle/moderate/strong): 20-25 points × tide_weight';
  RAISE NOTICE '  • No tide data: 15 points (neutral) × tide_weight';
  RAISE NOTICE '';
  RAISE NOTICE 'Species Examples:';
  RAISE NOTICE '  • Bass (tide_weight: 0.35): 40 × 0.35 = 14 points at perfect tide';
  RAISE NOTICE '  • Mackerel (tide_weight: 0.30): 40 × 0.30 = 12 points at perfect tide';
  RAISE NOTICE '  • Mullet (tide_weight: 0.30): 40 × 0.30 = 12 points at rising tide';
  RAISE NOTICE '';
