-- Enhancement 1: Add time-of-day scoring using diurnal_sensitivity
-- This enhancement requires zero external dependencies
-- Uses species.diurnal_sensitivity and current hour to boost dawn/dusk feeders

-- Step 1: Create helper function to categorize time of day
CREATE OR REPLACE FUNCTION get_time_of_day_category(target_hour integer)
RETURNS text
LANGUAGE plpgsql
IMMUTABLE
AS $$
BEGIN
  RETURN CASE
    -- Dawn: 5-7 AM
    WHEN target_hour IN (5, 6, 7) THEN 'dawn'
    -- Day: 8 AM - 5 PM
    WHEN target_hour >= 8 AND target_hour <= 17 THEN 'day'
    -- Dusk: 6-8 PM
    WHEN target_hour IN (18, 19, 20) THEN 'dusk'
    -- Night: 9 PM - 4 AM
    ELSE 'night'
  END;
END;
...
COMMENT ON FUNCTION get_time_of_day_category IS 'Categorizes hour (0-23) into dawn/day/dusk/night for diurnal sensitivity scoring';

-- Step 2: Update enhanced prediction function to include time-of-day scoring
...
RETURNS TABLE (
  ...
  depth_score integer,
  light_score integer,  -- NEW: Time-of-day score
...
BEGIN
  ...
  biogeochemical_enhancements AS (
    SELECT
      ...
      s.scientific_name,
      s.playful_bio_en,
      s.diurnal_sensitivity,  -- NEW: Query diurnal sensitivity
      s.light_weight,          -- NEW: Query light weight
  ...
  depth_matches AS (
    ...
    SELECT
      ...
      CASE
        ...
        WHEN s.depth_min_m IS NULL OR s.depth_max_m IS NULL THEN 10  -- No preference data
        WHEN user_depth_m BETWEEN s.depth_min_m AND s.depth_max_m THEN 20  -- Within optimal range
        WHEN user_depth_m BETWEEN (s.depth_min_m - 10) AND (s.depth_max_m + 10) THEN 15  -- Close to optimal
        WHEN user_depth_m < s.depth_min_m AND (s.depth_min_m - user_depth_m) <= 20 THEN 10  -- Shallow marginal
        WHEN user_depth_m > s.depth_max_m AND (user_depth_m - s.depth_max_m) <= 20 THEN 10  -- Deep marginal
  ...
  light_matches AS (
    -- NEW: Calculate time-of-day score based on diurnal sensitivity
    SELECT
      be.species_id,
      CASE
        ...
        -- Night: Nocturnal hunters (strong sensitivity) get bonus at night
  ...
  SELECT
    ...
    LEAST(100, GREATEST(0, 
      ...
      -- NEW: Light/time-of-day scoring (0-15)
    ...
    COALESCE(lm.light_score, 7)::integer as light_score,  -- NEW: Return light score
    (CASE
      WHEN DATE(be.latest_capture) = target_date THEN 15
      WHEN DATE(be.latest_capture) >= target_date - 1 THEN 13
      WHEN DATE(be.latest_capture) >= target_date - 3 THEN 11
  ...
  LEFT JOIN light_matches lm ON be.species_id = lm.species_id  -- NEW: Join light scores
  ORDER BY confidence DESC, be.name_en;
END;
$$;

COMMENT ON FUNCTION get_environmental_predictions_enhanced IS 'Enhanced confidence scoring with time-of-day awareness. Uses diurnal_sensitivity to boost dawn/dusk feeders during crepuscular hours and nocturnal species at night. Includes lat/lon-based substrate and depth scoring.';

-- Step 3: Update basic prediction function (without GPS) to include time-of-day scoring
...
COMMENT ON FUNCTION get_environmental_predictions_basic IS 'Basic confidence scoring with time-of-day awareness. Uses diurnal_sensitivity to boost dawn/dusk feeders during crepuscular hours and nocturnal species at night.';
...
DECLARE
  ...
  test_category text;
BEGIN
  -- Test time category calculation
  FOR test_hour IN 0..23 LOOP
    test_category := get_time_of_day_category(test_hour);
    RAISE NOTICE 'Hour %: %', LPAD(test_hour::text, 2, '0'), test_category;
  END LOOP;
  
  RAISE NOTICE '';
  RAISE NOTICE '✅ Time-of-day scoring enhancement installed!';
  RAISE NOTICE 'Both RPC functions now include light_score field.';
  RAISE NOTICE 'Dawn/dusk species (strong diurnal_sensitivity) will score +15 points during crepuscular hours.';
END;
$$;
