-- Add Biogeographic Filtering to Confidence V3
-- November 18, 2025
-- Critical Bug Fix: Pacific species appearing in Atlantic waters
-- Issue: get_fishing_confidence_v3 was returning ALL species without biogeographic filtering
-- Solution: Add biogeographic region filter based on rectangle's mapped regions
...
BEGIN
  ...
  -- **CRITICAL FIX: Added biogeographic region filtering**
  species_scores AS (
    SELECT
      s.id AS species_id,
    ...
    WHERE s.species_code IS NOT NULL
      ...
      AND (
        ...
        OR s.biogeographic_regions && ARRAY[rr.biogeographic_region]::TEXT[]
      )
  ),
...
COMMENT ON FUNCTION get_fishing_confidence_v3 IS 'Confidence V3 with biogeographic filtering AND regional seasonality (FIXED 2025-11-18). Filters species by biogeographic_regions array overlap with rectangle regions. Maps CMEMS regions to biogeographic regions (IBI→{BIS,IBR}, NWS→{NEA,NSEA,SCA}, BAL→BALT, MED→MED). Prevents Pacific species from appearing in Atlantic waters and vice versa.';
