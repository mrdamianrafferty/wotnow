-- Cast ALL VARCHAR columns to TEXT to match function return type declaration
-- Errors: "Returned type character varying(X) does not match expected type text"
...
BEGIN
  ...
  WITH base_enriched AS (
    SELECT
      s.species_code::TEXT,  -- Cast VARCHAR(10) to TEXT
      s.id::TEXT AS species_id,
      s.name_en::TEXT,  -- Cast VARCHAR(100) to TEXT
      s.scientific_name::TEXT,  -- Cast VARCHAR(100) to TEXT
      s.playful_bio_en,  -- Already TEXT
      s.guild,  -- Already TEXT
      s.species_badges,
      s.diurnal_sensitivity,  -- Already TEXT
      s.preferred_tide_stage,
      s.light_weight,
      s.temp_opt_c,
      s.flow_preference,  -- Already TEXT
      s.temp_weight,
      s.tide_weight,
      s.lunar_weight,
  ...
  SELECT
    ...
    be.species_id,  -- Already cast to TEXT in CTE
    be.name_en,  -- Already cast to TEXT in CTE
    be.scientific_name,  -- Already cast to TEXT in CTE
    LEAST(GREATEST(
...
COMMENT ON FUNCTION get_environmental_predictions_enhanced IS 'FINAL FIX: All VARCHAR columns (species_code, name_en, scientific_name) explicitly cast to TEXT. Uses cached_at for moon_cache. All database column references corrected.';
