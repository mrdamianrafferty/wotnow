-- Fix playful_bio column name (should be playful_bio_en)
...
-- Add helpful comment
COMMENT ON FUNCTION get_environmental_predictions_basic IS 'Enhanced confidence scoring with species-specific bio-band matching. All column name issues fixed: surface_temperature (not surfaceTemperature), playful_bio_en (not playful_bio), species_substrates.species_code (not species_id). Uses 7-day window for environmental data.';
