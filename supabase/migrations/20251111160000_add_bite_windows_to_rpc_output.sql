-- Add preferred_tide_stage and flow_preference to get_environmental_predictions_enhanced output
-- These fields are used by the frontend to generate bite windows showing optimal fishing times
...
RETURNS TABLE(
  ...
  preferred_tide_stage TEXT[],  -- NEW: For tide-based bite windows
  temp_opt_c NUMERIC[],
  flow_preference TEXT,  -- NEW: For flow-based bite windows
...
BEGIN
  ...
  SELECT center_lat, center_lon, biogeographic_region
  ...
  SELECT temperature_c, salinity_ppt, substrate, data_age_hours
  ...
  IF NOT FOUND THEN
    ...
    condition_record := ROW(NULL, NULL, NULL, NULL);
  END IF;
  ...
  WITH base_enriched AS (
    SELECT
      ...
      s.preferred_tide_stage,  -- NEW: Tide preferences for bite windows
      s.light_weight,
      s.temp_opt_c,
      s.flow_preference,  -- NEW: Flow preferences for bite windows
      s.temp_weight,
      s.tide_weight,
      s.lunar_weight,
      condition_record.temperature_c AS actual_temp,
      condition_record.salinity_ppt AS actual_salinity,
      COALESCE(substrate_type, condition_record.substrate) AS actual_substrate,
      COALESCE(depth_meters, 20) AS actual_depth,
      condition_record.data_age_hours
    FROM species s
    WHERE s.status = 'active'
      AND (
        rectangle_record.biogeographic_region = ANY(s.biogeographic_regions)
  ...
  SELECT
    ...
    be.preferred_tide_stage,  -- NEW: Return tide preferences
    be.temp_opt_c,
    be.flow_preference,  -- NEW: Return flow preferences
    be.light_weight,
    be.temp_weight,
    be.tide_weight,
    be.lunar_weight,
