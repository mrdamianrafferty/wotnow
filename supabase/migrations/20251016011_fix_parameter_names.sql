-- Fix parameter name mismatch and simplify bio-band matching
-- Issue: species_bio_bands uses "surface_temperature" but thresholds use "surfaceTemperature"
...
BEGIN
  ...
  confidence_scores AS (
    SELECT
      ...
      (
        (CASE WHEN EXISTS (SELECT 1 FROM species_bio_bands WHERE species_id = be.species_id) THEN 6 ELSE 0 END) +
        (CASE WHEN EXISTS (SELECT 1 FROM species_substrates WHERE species_id = be.species_id) THEN 4 ELSE 0 END) +
...
-- Add helpful comment
COMMENT ON FUNCTION get_environmental_predictions_basic IS 'Enhanced confidence scoring with species-specific bio-band matching. Fixed parameter name mismatch (surface_temperature vs surfaceTemperature) and uses 7-day window for environmental data to handle split snapshots.';
