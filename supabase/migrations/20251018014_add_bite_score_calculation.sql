-- Add bite_score calculation to get_environmental_predictions_enhanced
-- This implements the species-specific weighted bite score algorithm
...
BEGIN
  ...
  -- Normalize region for biogeographic filtering
  ...
  SELECT
    ...
    -- NEW: Bite Score Calculation
    -- Combines all factors with species-specific weights
    -- Focus on actionable fishing conditions (tide, light, weather, temp)
    LEAST(
      (
        COALESCE(tm.temp_score, 0) +  -- Already weighted by temp_weight
        -- Tide score (placeholder - will be 0 until tide data available)
        (0 * COALESCE(be.tide_weight, 0.30)) +
        -- Light/diurnal score (already weighted by light_weight in light_matches)
        COALESCE(lm.light_score, 0) +
        -- Lunar score (already weighted by lunar_weight)
        COALESCE(lnm.lunar_score, 0) +
        -- Weather score (already weighted by wind_weight + pressure_weight)
        COALESCE(wm.weather_score, 0) +
        -- Bio factors (smaller contribution to bite timing)
...
DO $$
BEGIN
  RAISE NOTICE '✅ Bite Score implemented in get_environmental_predictions_enhanced!';
  RAISE NOTICE '';
  RAISE NOTICE 'NEW: Bite Score Algorithm';
  RAISE NOTICE '═══════════════════════════════════════════════════════════';
  RAISE NOTICE '';
  RAISE NOTICE 'The bite score represents the optimal fishing time for each species';
  RAISE NOTICE 'based on species-specific behavioral patterns and environmental factors.';
  RAISE NOTICE '';
  RAISE NOTICE 'Components (species-weighted):';
  RAISE NOTICE '  • Temperature Score: Already weighted by temp_weight (0.08-0.35)';
  RAISE NOTICE '  • Tide Score: Weighted by tide_weight (0.30) [placeholder until tide data available]';
  RAISE NOTICE '  • Light/Diurnal Score: Weighted by light_weight (0.30)';
  RAISE NOTICE '  • Lunar Phase Score: Weighted by lunar_weight (0.05)';
  RAISE NOTICE '  • Weather Score: Wind + Pressure weighted (0.15 + 0.10)';
  RAISE NOTICE '  • Bio Factors: Chlorophyll + Oxygen + Salinity (5 percent contribution)';
  RAISE NOTICE '  • Habitat Bonus: Species-specific location bonuses (10 percent contribution)';
  RAISE NOTICE '';
  RAISE NOTICE 'Key Differences from Confidence Score:';
  RAISE NOTICE '  • Confidence = "Is this species likely HERE?" (environmental match)';
  RAISE NOTICE '  • Bite Score = "Is this species likely to FEED NOW?" (behavioral timing)';
  RAISE NOTICE '';
  RAISE NOTICE 'Species Weight Examples:';
  RAISE NOTICE '  • Bass (light-sensitive): high light_weight (0.30), high tide_weight (0.30)';
  RAISE NOTICE '  • Mackerel (temperature-driven): high temp_weight (0.35)';
  RAISE NOTICE '  • Wrasse (resident): low temp_weight (0.08), high light_weight (0.30)';
  RAISE NOTICE '';
  RAISE NOTICE 'Results are now ordered by: bite_score DESC, confidence DESC';
  RAISE NOTICE '';
END $$;
