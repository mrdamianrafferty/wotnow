-- Fix Confidence V2 filter to show all species with baseline scores
-- November 17, 2025
-- Changes:
-- 1. Remove base_availability > 0 filter (show non-commercial species)
-- 2. Add biogeographic region filtering (only show regionally-appropriate species)
-- 3. Give baseline scores to non-commercial species (catch_score=5)
-- 4. Give neutral scores to species with empty seasonal arrays (seasonal_score=5)
...
BEGIN
  ...
  -- Get biogeographic region for the rectangle
  rectangle_region AS (
    SELECT
      cmems_region,
      -- Map CMEMS region to biogeographic regions
      CASE
        WHEN cmems_region IN ('IBI', 'NWS', 'BAL') THEN 'NE_Atlantic'
        WHEN cmems_region = 'MED' THEN 'Mediterranean'
        ELSE NULL
      END AS biogeographic_region
    FROM ices_rectangles
    WHERE code = target_rectangle
    LIMIT 1
  ),
  ...
  species_scores AS (
    ...
    WHERE s.species_code IS NOT NULL
      AND r.biogeographic_region = ANY(s.biogeographic_regions)
  ),
  ...
  SELECT
    ...
    -- Component scores
    ns.base_availability::NUMERIC AS base_availability_score,
...
COMMENT ON FUNCTION get_fishing_confidence_v2 IS 'Fixed confidence scoring with biogeographic filtering. Shows all species with baseline scores (non-commercial get catch_score=5, empty seasonal arrays get seasonal_score=5). Filters species by biogeographic region compatibility (CMEMS regions mapped to biogeographic regions: IBI/NWS/BAL→NE_Atlantic, MED→Mediterranean). No filtering by base_availability.';
