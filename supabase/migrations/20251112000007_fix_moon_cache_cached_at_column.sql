-- Fix moon_cache column reference: use cached_at instead of created_at
...
BEGIN
  ...
  -- Fetch moon data using correct column names (FIXED: use cached_at, not created_at)
  ...
  WITH base_enriched AS (
    SELECT
      s.species_code::TEXT,  -- Explicit cast to TEXT
...
COMMENT ON FUNCTION get_environmental_predictions_enhanced IS 'FIXED: Use cached_at instead of created_at for moon_cache. Explicit type casting for species_code (VARCHAR(10) -> TEXT). All database column references corrected.';
