-- Fix temp_opt_c array handling in global predictions
-- temp_opt_c can be either a single numeric value OR a numeric array [min, max]
-- When it's an array, use the midpoint for optimal temperature
...
BEGIN
  ...
  IF has_grid_data THEN
    ...
    SELECT
      ...
      LEAST((fs.temp_score + fs.substrate_score + fs.depth_score + fs.habitat_bonus), 100)::integer as bite_score,
      fs.bio_band_score,
      fs.temp_score,
      fs.substrate_score,
      fs.depth_score,
