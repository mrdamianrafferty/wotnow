-- FINAL FIX: Cast ALL columns to TEXT including ENUM types (fish_guild, etc.)
...
BEGIN
  ...
  WITH base_enriched AS (
    SELECT
      ...
      s.playful_bio_en,
      s.guild::TEXT,  -- Cast ENUM fish_guild to TEXT
      s.species_badges,
      s.diurnal_sensitivity::TEXT,  -- Cast ENUM to TEXT
      s.preferred_tide_stage,
      s.light_weight,
      s.temp_opt_c,
      s.flow_preference::TEXT,  -- Cast ENUM to TEXT
      s.temp_weight,
      s.tide_weight,
      s.lunar_weight,
...
COMMENT ON FUNCTION get_environmental_predictions_enhanced IS 'COMPLETE FIX: ALL type casts applied - VARCHAR columns (species_code, name_en, scientific_name) + ENUM columns (guild, diurnal_sensitivity, flow_preference) cast to TEXT. Uses cached_at for moon_cache. All column references corrected.';
