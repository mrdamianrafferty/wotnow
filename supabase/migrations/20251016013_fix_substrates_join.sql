-- Fix species_substrates join (uses species_code not species_id)
...
-- Add helpful comment
COMMENT ON FUNCTION get_environmental_predictions_basic IS 'Enhanced confidence scoring with species-specific bio-band matching. Fixed parameter name mismatch (surface_temperature vs surfaceTemperature), uses 7-day window for environmental data, and fixed species_substrates join (uses species_code not species_id).';
