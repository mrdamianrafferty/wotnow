-- Fix species_code varchar length mismatch in RPC return type
-- Issue: Database column is varchar(10) but function returns varchar (unlimited)
-- Solution: Update return type to match database column definition
...
RETURNS TABLE (
  species_id uuid,
  species_code character varying(10),  -- ← FIXED: Added length to match database column
  name_en varchar,
...
DO $$
BEGIN
  RAISE NOTICE '✅ Fixed species_code varchar length mismatch (varchar(10) → varchar)';
END $$;
