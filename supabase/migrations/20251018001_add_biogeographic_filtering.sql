-- Add biogeographic filtering to prediction functions
-- This ensures Atlantic locations show Atlantic species, Mediterranean locations show Mediterranean species
-- Uses existing region data from species_advice.regions field

-- Step 1: Add biogeographic_regions column to species table (array to support multiple regions)
ALTER TABLE species ADD COLUMN IF NOT EXISTS biogeographic_regions TEXT[];

-- Step 2: Populate biogeographic_regions based on species_code mapping
-- These mappings come from speciesAdviceData.json regions field
-- Format: ARRAY['Atlantic', 'Mediterranean', 'North Sea'] etc.

-- Mediterranean specialists (warm water species)
UPDATE species SET biogeographic_regions = ARRAY['Mediterranean'] 
WHERE species_code IN ('sba', 'sbg', 'rmu') -- Gilthead, Black Seabream variants, Red Mullet (Med variants)
AND biogeographic_regions IS NULL;

-- Mediterranean + Southern Atlantic (Iberian transitional)
UPDATE species SET biogeographic_regions = ARRAY['Mediterranean', 'Atlantic', 'Bay of Biscay', 'IBI'] 
WHERE species_code IN ('bog', 'bogue') -- Bogue
AND biogeographic_regions IS NULL;

-- North Atlantic, North Sea, Baltic (cold water specialists)
UPDATE species SET biogeographic_regions = ARRAY['Atlantic', 'North Sea', 'Baltic', 'Norwegian waters'] 
WHERE species_code IN ('cod', 'haddock', 'coley', 'saithe', 'ling')
AND biogeographic_regions IS NULL;

-- Cosmopolitan species (Atlantic, Mediterranean, North Sea)
UPDATE species SET biogeographic_regions = ARRAY['Atlantic', 'Mediterranean', 'North Sea', 'Celtic Sea', 'English Channel', 'Bay of Biscay', 'IBI'] 
WHERE species_code IN ('bass', 'seabass', 'bss', 'jdo', 'mul', 'gurnard', 'gar')  -- Sea Bass, John Dory, Mullet, Gurnard, Garfish
AND biogeographic_regions IS NULL;

-- Atlantic + North Sea (temperate species)
UPDATE species SET biogeographic_regions = ARRAY['Atlantic', 'North Sea', 'Celtic Sea', 'English Channel', 'Irish Sea', 'Bay of Biscay'] 
WHERE species_code IN ('ple', 'plaice', 'dab', 'flounder', 'fle', 'turbot', 'brill', 'brl', 'whiting', 'whg', 'pollack', 'pol', 'mackerel', 'mac')
AND biogeographic_regions IS NULL;

-- Default: All remaining species get broad Atlantic coverage (conservative approach - show everywhere until we have specific data)
UPDATE species SET biogeographic_regions = ARRAY['Atlantic', 'North Sea', 'Celtic Sea', 'English Channel', 'Irish Sea', 'Bay of Biscay', 'IBI', 'Mediterranean'] 
WHERE biogeographic_regions IS NULL;

-- Step 3: Update basic prediction function to filter by biogeographic region
...
BEGIN
  ...
  WITH recent_conditions AS (
    -- Get most recent environmental data within 7 days
    SELECT 
      rectangle_code,
  ...
  biogeochemical_enhancements AS (
    ...
    WHERE s.name_en IS NOT NULL
      -- BIOGEOGRAPHIC FILTER: Match species regions with rectangle region
      AND (
        rectangle_region IS NULL  -- No filtering if rectangle has no region
        OR s.biogeographic_regions IS NULL  -- Include species with no region data (for now)
        OR rectangle_region = ANY(s.biogeographic_regions)  -- Exact region match
        -- Allow some regional overlap for transitional zones
        OR (rectangle_region LIKE '%IBI%' AND 'IBI' = ANY(s.biogeographic_regions))
        OR (rectangle_region LIKE '%Biscay%' AND 'Bay of Biscay' = ANY(s.biogeographic_regions))
        OR (rectangle_region LIKE '%Atlantic%' AND 'Atlantic' = ANY(s.biogeographic_regions))
        OR (rectangle_region LIKE '%Mediterranean%' AND 'Mediterranean' = ANY(s.biogeographic_regions))
        OR (rectangle_region LIKE '%North Sea%' AND 'North Sea' = ANY(s.biogeographic_regions))
        OR (rectangle_region LIKE '%Celtic%' AND 'Celtic Sea' = ANY(s.biogeographic_regions))
        OR (rectangle_region LIKE '%Irish%' AND 'Irish Sea' = ANY(s.biogeographic_regions))
        OR (rectangle_region LIKE '%English Channel%' AND 'English Channel' = ANY(s.biogeographic_regions))
      )
  ),
  bio_band_matches AS (
  ...
  SELECT
    ...
    LEAST(100, GREATEST(0, 
      ...
      -- Temperature scoring (0-25) - Using bio-band matching for temperature
      (CASE 
    ...
    12::integer as substrate_score,  -- Default substrate score (no GPS)
...
COMMENT ON FUNCTION get_environmental_predictions_basic IS 'Biogeographically-aware confidence scoring with time-of-day awareness. Filters species by temperature suitability to prevent Mediterranean species appearing in Atlantic locations and vice versa.';
