-- COMPREHENSIVE FIX: Replace get_environmental_predictions_enhanced with correct column references
-- This migration fixes ALL database column mismatches found in the RPC function:
-- 1. ices_rectangles: biogeographic_region → region, code → rectangle_code
-- 2. findr_conditions_latest: temperature_c → sea_temp_c/water_temp_c, salinity_ppt → salinity_psu, substrate removed, data_age_hours calculated
-- 3. moon_cache: phase → moon_phase_name, illumination → moon_illumination_pct, date → local_date
-- 4. species: species_id → id, status column removed (doesn't exist)
...
BEGIN
  -- FIXED: Fetch rectangle metadata using correct columns
  SELECT center_lat, center_lon, region
  INTO rectangle_record
  FROM ices_rectangles
  WHERE rectangle_code = target_rectangle  -- FIXED: was "code"
  LIMIT 1;
  ...
  rectangle_region := rectangle_record.region;  -- FIXED: was biogeographic_region

  -- FIXED: Fetch environmental conditions using correct column names
  SELECT
    COALESCE(water_temp_c, sea_temp_c) as temperature,  -- FIXED: was temperature_c
    salinity_psu,  -- FIXED: was salinity_ppt
    EXTRACT(EPOCH FROM (NOW() - captured_at)) / 3600 as age_hours  -- FIXED: calculate from captured_at
  ...
  -- FIXED: Fetch moon data using correct column names
  SELECT moon_phase_name, moon_illumination_pct  -- FIXED: was phase, illumination
  INTO v_moon_phase, v_moon_illumination
  FROM moon_cache
  WHERE local_date = target_date  -- FIXED: was date
  LIMIT 1;
  ...
  WITH base_enriched AS (
    SELECT
      s.species_code,
      s.id::TEXT AS species_id,  -- FIXED: was s.species_id::TEXT
      s.name_en,
      s.scientific_name,
      s.playful_bio_en,
      s.guild,
      ...
      s.tide_weight,
      s.lunar_weight,
      v_water_temp_c AS actual_temp,  -- FIXED: use variable
      v_salinity_psu AS actual_salinity,  -- FIXED: use variable
      substrate_type AS actual_substrate,  -- FIXED: substrate column doesn't exist in table
      COALESCE(depth_meters, 20) AS actual_depth,
      v_data_age_hours AS data_age_hours  -- FIXED: use variable
    FROM species s
    WHERE  -- FIXED: removed "s.status = 'active'" condition (column doesn't exist)
      rectangle_region = ANY(s.biogeographic_regions)  -- FIXED: use variable
...
COMMENT ON FUNCTION get_environmental_predictions_enhanced IS 'COMPREHENSIVE FIX: All database column references corrected. Uses: ices_rectangles.region/rectangle_code, findr_conditions_latest (sea_temp_c/water_temp_c, salinity_psu, calculated data age), moon_cache (moon_phase_name, moon_illumination_pct, local_date), species.id (no status filter).';
