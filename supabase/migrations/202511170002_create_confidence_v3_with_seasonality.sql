-- Confidence V3 with Regional Seasonality Integration
-- November 17, 2025
--
-- Changes from V2:
-- 1. Integrates species_region_seasonality table for per-region seasonal adjustments
-- 2. Uses ices_rectangles.region (not biogeographic_region mapping)
-- 3. Multiplies confidence by seasonal_weight * availability_multiplier
-- 4. Returns seasonal debug fields (phase, weight, multiplier, source, confidence)
-- 5. Keeps V2 function intact (non-breaking)

-- Create index for efficient seasonality lookups
...
BEGIN
  ...
  -- Get region for the rectangle (use .region, not cmems_region)
  rectangle_region AS (
    SELECT
  ...
  with_seasonality AS (
    SELECT
      ...
      CASE
        ...
        ELSE 0.0 -- explicitly out of season
...
COMMENT ON FUNCTION get_fishing_confidence_v3 IS 'Confidence V3 with regional seasonality integration. Uses species_region_seasonality table for per-region seasonal weights. Joins on (species.id, ices_rectangles.region). Returns seasonally-adjusted confidence_percent multiplied by seasonal_weight * availability_multiplier. Includes debug fields: seasonal_phase, seasonal_weight, availability_multiplier, source, source_confidence.';
