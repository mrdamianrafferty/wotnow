...
| --- | --- | --- | --- |
| `/api/findr/predictions` | POST | Validates rectangle + date, reads cache (`findr_prediction_sessions`), calls RPC functions in cascade: (1) `get_environmental_predictions_enhanced` (PRIMARY: proven ICES rectangle-based, queries `findr_conditions_latest` from twice-daily Copernicus ingestion), (2) `get_global_fishing_predictions` (FALLBACK: global grid-based for non-ICES areas, new as of Nov 5 2025). Augments with localized names from `species` table and applies seasonality multipliers. | Uses service client via server-side; fetches EMODnet bathymetry/substrate, MET Norway weather, tide data in parallel; writes cache entries with TTL; warns if schema missing. |
