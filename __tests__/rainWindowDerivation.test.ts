...
function hours(date: string, perHour: number[]) {
  return {
...
function payload(date: string, perHour: number[]) {
  return {
    daily: {
      time: [date],
...
async function windowFor(perHour: number[]): Promise<string | undefined> {
  const g = global as unknown as { fetch: jest.Mock };
  g.fetch = jest.fn().mockResolvedValue({
    ok: true, status: 200, json: async () => payload('2026-07-15', perHour),
  });
