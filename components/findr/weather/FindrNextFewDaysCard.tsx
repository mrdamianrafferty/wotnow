...
// Lightweight inline WindDirectionIcon (arrow that rotates by deg)
interface WindDirIconProps {
  deg?: number;
  size?: number;
  className?: string;
}
const WindDirectionIcon: React.FC<WindDirIconProps> = ({ deg = 0, size = 16, className = "" }) => {
  const rotation = typeof deg === 'number' && Number.isFinite(deg) ? deg : 0;
  const px = `${size}px`;
  return (
    <svg
      width={px}
    ...
    >
      <g fill="currentColor">
        <path d="M12 2l3.5 7H8.5L12 2z" />
        <rect x="11" y="9" width="2" height="11" rx="1" />
      </g>
    </svg>
  );
};
