...
/** Readings, as `/api/unified-weather` returns them. Every field optional. */
interface Readings {
  hourly?: Array<Record<string, number | string | undefined>>;
  daily?: Array<Record<string, number | string | undefined>>;
 ...
 *
 * Reads the `unified-weather` shape defensively — every field is optional there
 * and a missing one must produce no row, never "undefined" or a zero. A
 * fabricated reading in a drawer whose whole job is to be checkable is the one
 * unforgivable bug in this file.
 */
function rowsFor(section: RankedSection, r: Readings | null): Row[] {
  if (!r) return [];
  const now = (r.hourly?.[0] ?? {}) as Record<string, number | undefined>;
  const today = (r.daily?.[0] ?? {}) as Record<string, number | undefined>;
  ...
  switch (section.id) {
    case 'wind':
      return [
        ...keep('Now', n(now.wind_speed ?? now.windSpeed, ' km/h')),
        ...keep('Gusting to', n(today.wind_gust ?? today.windGust, ' km/h')),
        ...keep('Direction', n(today.wind_deg ?? today.windDeg, '°')),
      ];
    case 'rain':
      return [
        ...keep('Today', n(today.rain ?? today.precipitation, ' mm', 1)),
        ...keep('Hours of it', n(today.precipitation_hours, ' h')),
        ...keep('Chance', n(typeof today.pop === 'number' ? today.pop * 100 : undefined, '%')),
      ];
    case 'temperature':
      return [
        ...keep('Now', n(now.temp ?? now.temperature, '°')),
        ...keep('High', n(today.temp_max ?? today.tempMax, '°')),
        ...keep('Low', n(today.temp_min ?? today.tempMin, '°')),
      ];
    case 'sea': {
      const m = (r.marine ?? {}) as Record<string, number | undefined>;
      return [
        ...keep('Wave height', n(m.waveHeight ?? m.wave_height, ' m', 1)),
        ...keep('Swell period', n(m.swellPeriod ?? m.swell_period, ' s')),
        ...keep('Sea temperature', n(m.waterTemperature ?? m.sea_temperature, '°', 1)),
      ];
    }
    case 'tide': {
      const t = (r.tides ?? {}) as Record<string, unknown>;
      const next = Array.isArray(t.extremes) ? (t.extremes as Array<Record<string, unknown>>).slice(0, 2) : [];
      return next.flatMap((e) => {
        const when = typeof e.time === 'string' ? e.time.slice(11, 16) : null;
        const kind = typeof e.type === 'string' ? e.type : 'Tide';
        return when ? [{ label: kind[0].toUpperCase() + kind.slice(1), value: when }] : [];
      });
    }
    case 'sky':
      return [
        ...keep('Cloud', n(today.clouds ?? now.clouds, '%')),
        ...keep('Visibility', n(typeof now.visibility === 'number' ? now.visibility / 1000 : undefined, ' km', 1)),
        ...keep('UV', n(today.uvi ?? now.uvi, '')),
      ];
    case 'air': {
      const aq = r.airQuality;
      return [
        ...keep('Humidity', n(now.humidity ?? today.humidity, '%')),
        ...keep('Air quality index', n(aq?.aqi, '')),
        ...keep('PM2.5', n(aq?.components?.pm2_5, ' µg/m³', 1)),
      ];
    }
    case 'ground': {
      const s = r.soil ?? {};
      return [
        ...keep('Soil at 6 cm', n(s.temp6cm, '°')),
        ...keep('Moisture, top layer', n(typeof s.moisture0to1 === 'number' ? s.moisture0to1 * 100 : undefined, '%')),
      ];
    }
    case 'night':
      return keep('Moon', n(typeof today.moon_phase === 'number' ? today.moon_phase * 100 : undefined, '% through'));
    default:
      return [];
  }
}
