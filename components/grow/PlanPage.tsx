...
export function PlanPage() {
  ...
  const signalToEvent = useCallback((signal: LocalSignal): TimelineEvent => {
    ...
    return {
      id: `signal-${signal.id}`,
      type,
      title: `${emoji} ${signal.title || signal.name || 'Alert'}`,
      description: `${signal.description}${signal.advice ? ` ${signal.advice}` : (signal.recommendation ? ` ${signal.recommendation}` : '')}`,
  ...
  const weatherTaskToEvent = useCallback((task: WeatherTask): TimelineEvent => {
    ...
    return {
      ...
      description: `${task.description}${task.recommendation ? ` ${task.recommendation}` : ''}`,
      dateLabel: validFrom.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
      startDate: validFrom,
      endDate: validUntil,
